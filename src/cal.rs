  ...
  match task.get_start() {
    Some(start) => {
      ...
      match rep {
        ...
        Repetition::Yearly => {
          start <= check && start.day() == check.day() && start.month() == check.month()
        }
      }
    }
    None => false
  ...
  match get_prev_sunday(date) {
    ...
    Some(start) => {
      ...
      match next_month(&start) {
        None => println!(
          "something went wrong checking the length of month: {:?}",
          start
        ),
        Some(next) => {
          // get the next month so we know if we roll over
          let length = next
            .signed_duration_since(NaiveDate::from_ymd(start.year(), start.month(), 1))
            .num_days();
          print_weekdays();
          for i in 0..7 {
            ...
            let day_justified = if occurs {
              Yellow.bold().paint(day_justified).to_string()
            } else {
              String::from(day_justified)
            };
            print!("{}", day_justified);
          }
          println!();
...
pub fn print_year(start: NaiveDate, tasks: Vec<impl TaskLike>) {
  ...
  match next_year(&start) {
    ...
    Some(next) => {
      let length: u32 = next
        .signed_duration_since(NaiveDate::from_yo(year, 1))
        .num_days() as u32;
