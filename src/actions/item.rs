...
pub fn get_items(conn: &PgConnection, filters: ItemFilter) -> Result<ItemVec, diesel::result::Error> {
  ...
  match (filters.created_before, filters.created_after) {
    (Some(start), Some(end)) => {
      ...
      if (start > end) { // honestly let's just throw a 401 here
        real_start = end;
        real_end = start;
      }
      let real_start = real_start.naive_utc();
      let real_end = real_end.naive_utc();
      query = query.filter(created_at.ge(real_start))
  ...
  match (filters.updated_before, filters.updated_after) {
    (Some(start), Some(end)) => {
      ...
      let real_start = real_start.naive_utc();
      let real_end = real_end.naive_utc();
      query = query.filter(updated_at.ge(real_start))
  ...
  let selected_items = query
    .load::<Item>(conn)?;

  // post-load modifications (Date filtering)

  match (filters.occurs_after, filters.occurs_before) {
    (Some(start), Some(end)) => { response.items = occurs_between(selected_items, start, end); },
    // (Some(start), None) => (), //
    // (None, Some(start)) => (),
...
pub fn get_parents_by_child_id(item_id: i32, conn: &PgConnection) -> Result<Vec<Item>, diesel::result::Error> {
  use crate::schema::item_references;
...
pub fn get_children_by_parent_id(item_id: i32, conn: &PgConnection) -> Result<Vec<Item>, diesel::result::Error> {
  use crate::schema::item_references;
...
pub fn get_references_by_id(item_id: i32, conn: &PgConnection) -> Result<Option<ItemResponse>, diesel::result::Error> {
