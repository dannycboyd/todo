...
fn task_parts() -> String {
  ...
  format!(
    "start_date, repetition, [text_data]\n\tstart_date: {date}\n\trepetition: {rep}\n\t{text}",
    date = date_parts(),
    rep = rep_parts(),
    text = parts3
  )
...
pub fn detailed_help(cmd: Option<String>) {
  match cmd {
    Some(cmd) => {
      match cmd.as_ref() {
        // fix this mess so it does a boilerplate "usage: {}", cmd_help() sort of thing
        ...
        "do" => println!(
          "{}\nMark a task as completed, with optional date and set as finished",
          do_help()
        ),
        "list" => println!("{}", list()),
